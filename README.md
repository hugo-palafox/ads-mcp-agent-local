...
For automated smoke validation or a replayable live-demo transcript, use `python scripts/feature_smoke_test.py --mode smoke|demo`.
...
- Demo mode resets learned memory at the start, then walks through readiness, discovery, reads, clarification, learning, follow-up improvement, and guarded writes.
- It captures a real unknown-term clarification before teaching and repeats the same intent after alias learning.
- It writes three artifacts under `artifacts/smoke-tests/`: raw JSON, Markdown transcript, and replay command list.
- Use `--mode smoke` when you want strict pass/fail validation instead of the presentation-oriented transcript flow.
