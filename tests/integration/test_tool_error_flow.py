...
def test_tool_error_flow() -> None:
    llm = FakeLLMClient(
        [
            ModelResponse(content=None, tool_calls=[ModelToolCall(id="1", name="read_memory", arguments={})], raw={}),
            ModelResponse(content="I could not verify the machine state because the read failed.", tool_calls=[], raw={}),
        ]
    )
    bridge = AdsToolBridge(FakeMcpClient(errors={"read_memory": RuntimeError("PLC offline")}))  # type: ignore[arg-type]
    ...
    assert result.answer == "I could not verify the machine state because the read failed."
    assert result.tool_trace[0].ok is False
    assert result.tool_trace[0].error == "PLC offline"
